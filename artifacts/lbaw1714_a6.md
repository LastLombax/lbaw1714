...
<table>
 <tr>
  ...
  <td>UPDATE01</td>
   ...
   <tr>
   <td><strong>Query Description</strong></td>
   <td>Updating Event's Information</td>
   </tr>
  <tr>
  <td><strong>Query Frequency</strong></td>
  <td>hundreds per day</td>
...
```sql
UPDATE "event"
  SET name = $title, description = $obs, imagePath = $year, country = $country, city = $city
  WHERE "idevent" = $id; 
```
 
 Eventos:
 inserir evento
 atualizar evento info
 criar comentários
 convidar membros
 juntar-se ao evento
...
Report:
 evento, comunidade ou outro membro inserido
