...
<table>
 <tr>
  ...
  <td>TRIGGER04</td>
   ...
   <tr>
   <td><strong>Trigger Description</strong></td>
   <td>When a User is banned from a community, their comments are set to null (in order to be flagged as such by the server when showing the page)</td>
   </tr>
</table>

```sql
CREATE FUNCTION setUser() RETURNS TRIGGER AS
$BODY$
BEGIN
