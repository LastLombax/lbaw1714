...
<table>
 <tr>
  ...
  <td>SELECT04</td>
   ...
   <tr>
   <td><strong>Query Description</strong></td>
   <td>Advise that Member has a ticket type of X for a specific event (this will be a popup for an event page)</td>
   </tr>
  <tr>
  <td><strong>Query Frequency</strong></td>
...
```sql
SELECT friend.idf2 FROM friend WHERE friend.idf1 = $_SESSION['currentMember'] AND friend.accepted = true
LIMIT $selectedLimit OFFSET $selectedOffset
```
...
```sql
SELECT name, username, profilepicture FROM memebr WHERE
...
<table>
 <tr>
  ...
  <td>IDX01</td>
   ...
   <td>The query related to this index uses several JOINs, and as such a lot of tuples have repeated data in order to accommodate data from many-to-many relations.</td>
   </tr>
</table>

 ```sql
    CREATE INDEX member_username_email ON "member" USING btree (username, email);
  ```
...
<table>
 ...
 ```sql
    CREATE INDEX community_id_idx ON "community" USING btree (idcommunity);
  ```
 ...
 <tr>
  ...
  <td>IDX03</td>
   ...
   <td>No</td>
      ...
      <tr>
   <td><strong>Justification</strong></td>
   <td>Users will search for data using its human-readable name.</td>
   </tr>
</table>

```sql
CREATE INDEX search_community_idx ON community USING BTREE (to_tsvector('english', name))
```
...
```sql
CREATE INDEX search_event_idx ON event USING BTREE (to_tsvector('english', name))
```
...
//TO MERGE AND FIX Trigger 4 and 5
<table>
 <tr>
  <td><strong>Trigger Reference</strong></td>
  <td>TRIGGER04</td>
...
<table>
 <tr>
  ...
  <td>TRIGGER05</td>
   ...
   <tr>
   <td><strong>Trigger Description</strong></td>
   <td>After buying a ticket, available ticket quantity is decreased</td>
   </tr>
</table>

```sql
CREATE FUNCTION decreaseTicketAmount() RETURNS TRIGGER AS
$BODY$
BEGIN
  UPDATE "ticketType"
  SET availablequantity = old.availablequantity - invoice.quantity
  SELECT quantity
  FROM "invoice"
  WHERE (new.idinvoice = invoice.idinvoice);
  RETURN NEW;
END
$BODY$
LANGUAGE plpgsql;
...
CREATE TRIGGER buyTicketBefore
  ...
  FOR EACH ROW
    EXECUTE PROCEDURE decreaseTicketAmount();
```
///////////////////////////////////////////////////////
