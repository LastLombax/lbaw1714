...
| R13                | Notification        | thousands | hundreds per day        |
| R14                | Report        | thousands | units per week   |
| R15                | Ticket        | hundreds | hundreds per week       |
...
<table>
 <tr>
  ...
  <td>SELECT04</td>
   ...
   <tr>
   <td><strong>Query Description</strong></td>
   <td>Advise that Member has a ticket type of X for a specific event (this will be a popup in the event page)</td>
   </tr>
  <tr>
  <td><strong>Query Frequency</strong></td>
...
```sql
SELECT e1.name, e1.description, e1.imagePath, e1.startday, e1.endday, e1.starttime, e1.endtime, e1.country, e1.city, e1.address, category
FROM "event" e1
INNER JOIN LATERAL(
...
<table>
 <tr>
  ...
  <td>SELECT20</td>
   ...
   <tr>
   <td><strong>Query Description</strong></td>
   <td>event comments from scpecific event</td>
   </tr>
  <tr>
  <td><strong>Query Frequency</strong></td>
  <td>thousands per day</td>
...
```sql
SELECT ticketType, ticketDescription, ticketPrice, invoiceTaxPayerNr, invoiceName, invoiceQuantity, invoiceAmount, invoiceAddress, invoiceDate
FROM ticket t1
INNER JOIN LATERAL(
